...
class PINN:
    ...
    def build(self):
        ...
        # boundary condition input: (t, x=L)
        ...
        u_bnd = self.network(tx_bnd2)                # dirichlet
        _, _, du_dx_bnd, _, _ = self.grads(tx_bnd1)  # neumann

        # build the PINN model for the wave equation
        return tf.keras.models.Model(
            inputs=[tx_eqn, tx_ini, tx_bnd2, tx_bnd1],
            outputs=[u_eqn, u_ini, du_dt_ini, u_bnd, du_dx_bnd])
